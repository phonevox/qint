...
puxar_arquivos_integracao () {
    ...
    realiza_sftp () {
        checa_erro_sftp () {
            if [[ $status -ne 0 ]]; then
                log "[$(colorir "amarelo" "SFTP")] $(colorir "vermelho" "ERROR") - Ocorreu algum erro ao tentar puxar os arquivos do host remoto. (status:$status) (host:$SFTP_USER@$SFTP_HOST_REMOTO)"
