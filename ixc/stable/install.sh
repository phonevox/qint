...
puxar_arquivos_integracao () {
    ...
    realiza_scp () {
        checa_erro_scp () {
            if [[ $status -ne 0 ]]; then
                log "[$(colorir "amarelo" "SCP")] $(colorir "vermelho" "ERROR") - Ocorreu algum erro ao tentar puxar os arquivos do host remoto. (status:$status) (host:$SCP_USER@$SCP_HOST_REMOTO)"
        ...
        log "[$(colorir "amarelo" "SCP")] Puxando \"$(colorir "magenta_claro" "remote:$SCP_PATH_TO_TYPE/$TIPO_INTEGRACAO/$SCP_VERSAO_INTEGRACAO")\" -> \"$(colorir "magenta_claro" "local:$INTEGRACAO_DIR_TIPO_LOCAL")\""
        ...
        scp -r $SCP_USER@$SCP_HOST_REMOTO:$SCP_PATH_TO_TYPE/$TIPO_INTEGRACAO/$SCP_VERSAO_INTEGRACAO $INTEGRACAO_DIR_TIPO_LOCAL || local status=$? # EU NÃO VOU AUTOMATIZAR O PROCESSO DE REPASSAR À SENHA AO SCP. ISSO É EXTREMAMENTE INSEGURO.
...
alterando_parametros () {
    ...
    parametros_php () {
        ...
        log "[$(colorir "amarelo" "PHP")] \$server_local = $IP_HOST"
        sed -i "s|\$server_local = ''|\$server_local = '$IP_HOST'|" $LOCAL_PHP_CONFIG || { log "sed \"\$server_local\" falhou."; exit 1; }
    ...
    parametros_macro () {
        ...
        "Set(ocorrencia_outros_assuntos=$ID_ASSUNTO_GERAL)"

        substituir_valor "$LOCAL_MACROS_CONF" \
        "Set(ocorrencia_comercial=XXX)" \
        "Set(ocorrencia_comercial=$ID_ASSUNTO_COMERCIAL)"

        substituir_valor "$LOCAL_MACROS_CONF" \
        "Set(ocorrencia_suporte=XXX)" \
        "Set(ocorrencia_suporte=$ID_ASSUNTO_SUPORTE)"

        substituir_valor "$LOCAL_MACROS_CONF" \
        "Set(ocorrencia_financeiro=XXX)" \
        "Set(ocorrencia_financeiro=$ID_ASSUNTO_FINANCEIRO)"

        substituir_valor "$LOCAL_MACROS_CONF" \
        "Set(setor_outros_assuntos=XXX)" \
        "Set(setor_outros_assuntos=$ID_DEPARTAMENTO_GERAL)"

        substituir_valor "$LOCAL_MACROS_CONF" \
        "Set(setor_comercial=XXX)" \
        "Set(setor_comercial=$ID_DEPARTAMENTO_COMERCIAL)"

        substituir_valor "$LOCAL_MACROS_CONF" \
        "Set(setor_suporte=XXX)" \
        "Set(setor_suporte=$ID_DEPARTAMENTO_SUPORTE)"

        substituir_valor "$LOCAL_MACROS_CONF" \
        "Set(setor_financeiro=XXX)" \
        "Set(setor_financeiro=$ID_DEPARTAMENTO_FINANCEIRO)"
...
ajustes_finais () {
    ...
    local __LINHA_DE_INCLUDE="#include $TIPO_INTEGRACAO/$__NOME_ARQUIVO_INCLUDE" # Qual o texto que deve ser inserido no extensions_custom.conf pra rodar o arquivo de include?
    ...
    orientacoes () {
        ...
        echo -e ""
        echo -e "$(colorir "magenta" "inicio-ixcsoft"),$(colorir "verde" "s"),$(colorir "magenta" "1")"
        echo -e "URA - IXCSOFT - INICIO"
        echo -e ""
        echo -e "$(colorir "magenta" "feriado-ixcsoft"),$(colorir "verde" "s"),$(colorir "magenta" "1")"
        echo -e "URA - IXCSOFT - FERIADO"
        echo -e ""
        echo -e "$(colorir "magenta" "fechado-ixcsoft"),$(colorir "verde" "s"),$(colorir "magenta" "1")"
        echo -e "URA - IXCSOFT - FECHADO"
        echo -e ""
        echo -e "$(colorir "magenta" "from-internal"),$(colorir "verde" "\${departamento}"),$(colorir "magenta" "1")"
        echo -e "URA - IXCSOFT - FILA"
        echo -e ""
